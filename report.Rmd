...
convert_bytes <- function(x, to_unit) {
  units <- list(
    MiB = 2,
    GiB = 3,
    TiB = 4
  )
  return(x / (1024^units[[to_unit]]))
}
...
total_usage_tb <- user_dat %>%
  ...
  mutate(disk_usage_tb = convert_bytes(TotalBytes, "TiB")) %>%
  pull(disk_usage_tb)
  ...
  plots <- summary_metrics %>% lapply(function(y_metric) {
    ...
    if (y_metric == "TotalBytes" | y_metric == "DuplicateBytes") {
      ...
      summary_dat_folder <- summary_dat_folder %>%
        mutate("{new_metric_name}" := convert_bytes(eval_tidy(data_sym(y_metric)), to_unit))
