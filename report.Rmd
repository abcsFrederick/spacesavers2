...
p <- disk_usage %>% 
  ...
  pivot_longer(c(used_tib, size_tib, avail_tib), names_to = 'metric') %>% 
  mutate(value = round(value, 2)) %>%
    ...
    geom_line(alpha = 0.7) +
    geom_point(aes(text = glue("{value} ({`Use%`})"))) +
  scale_x_datetime(labels = date_format("%b %Y")) +
  scale_color_brewer(palette = "Set1",
                     labels = c(size_tib = 'Size', used_tib = "Used", avail_tib = "Avail"),
                     breaks = c('size_tib', 'used_tib', 'avail_tib') # enforce order
                     ) +
    labs(y = 'TiB', x = '')
p
...
card(
  ...
  datatable(allusers_summary, fillContainer = TRUE, fillContainer = TRUE)
)
```
